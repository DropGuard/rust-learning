...
fn handle_echo_client(mut stream: TcpStream) -> io::Result<()> {
...
fn example5_udp_client() -> io::Result<()> {
    let socket = UdpSocket::bind("127.0.0.1:0")?; // 绑定到任意可用端口
...
fn example6_socket_address() {
    ...
    // 判断 IP 类型
    match addr {
        SocketAddr::V4(v4) => println!("  是 IPv4"),
        SocketAddr::V6(v6) => println!("  是 IPv6"),
    }
}
